...
// BACS ISO 20022 SEPA Credit Transfer is similar to SEPA Credit Transfer but with some differences; we are using the same library for both.
...
(function(exports) {
   ...
   */
  function SepaPaymentInfo(painFormat, options) {
    const {paymentType, currency} = (options || {});
    this._painFormat = painFormat;
    this.method = painFormat.indexOf('pain.001') === 0 ? PaymentInfoTypes['Transfer' + ((paymentType === 'BACS') ? 'BACS' : '')] : PaymentInfoTypes.DirectDebit;
    this._payments = [];
    this._paymentType = paymentType || 'SEPA';
  ...
  SepaPaymentInfo.prototype = {
    ...
    createTransaction: function() {
      return new SepaTransaction(this._painFormat, {paymentType: this._paymentType, currency: this._currency});
    },
    ...
    validate: function() {
      ...
      if (this[pullFrom + 'IBAN']) {
        assert_iban(this[pullFrom + 'IBAN'], pullFrom + 'IBAN');
        assert_length(this[pullFrom + 'BIC'], [0, 8, 11], pullFrom + 'BIC');
        var countryMatches = (this[pullFrom + 'BIC'].length === 0 || this[pullFrom + 'BIC'].substr(4, 2)
          .replace(/IM|GG|JE/, 'GB') // UK territories
          .replace(/GF|GP|MQ|RE|PF|TF|YT|NC|BL|MF|PM|WF/, 'FR') // French territories
          === this[pullFrom + 'IBAN'].substr(0, 2));
        assert(countryMatches, 'country mismatch in BIC/IBAN');
      } else {
        // validate BACS account and sort code
        assert_bacs(this[pullFrom + 'ACCOUNT'], this[pullFrom + 'SORTCODE'], pullFrom);
      }

      assert_length(this._payments.length, 1, null, '_payments');

      // Structured address validation for pain.001.001.09
      if (this._painFormat === 'pain.001.001.09') {
        // Trim all fields
        const trimField = (v) => typeof v === 'string' ? v.trim() : v;
        this[pullFrom + 'StreetName'] = trimField(this[pullFrom + 'StreetName']);
        this[pullFrom + 'BuildingNumber'] = trimField(this[pullFrom + 'BuildingNumber']);
        this[pullFrom + 'BuildingName'] = trimField(this[pullFrom + 'BuildingName']);
        this[pullFrom + 'PostCode'] = trimField(this[pullFrom + 'PostCode']);
        this[pullFrom + 'TownName'] = trimField(this[pullFrom + 'TownName']);
        this[pullFrom + 'CountrySubDivision'] = trimField(this[pullFrom + 'CountrySubDivision']);
        this[pullFrom + 'Country'] = trimField(this[pullFrom + 'Country']);
        // Length checks
        assert_length(this[pullFrom + 'StreetName'], null, 70, pullFrom + 'StreetName');
        assert_length(this[pullFrom + 'BuildingNumber'], null, 16, pullFrom + 'BuildingNumber');
        assert_length(this[pullFrom + 'BuildingName'], null, 70, pullFrom + 'BuildingName');
        assert_length(this[pullFrom + 'PostCode'], null, 16, pullFrom + 'PostCode');
        assert_length(this[pullFrom + 'TownName'], null, 70, pullFrom + 'TownName');
        assert_length(this[pullFrom + 'CountrySubDivision'], null, 70, pullFrom + 'CountrySubDivision');
        // Country code must be exactly 2 uppercase letters
        assert_length(this[pullFrom + 'Country'], null, 2, pullFrom + 'Country');
      } else {
        // Unstructured address checks for other formats
    ...
    toXML: function(doc) {
      ...
      if (this._paymentType === 'BACS') {
        // this is obtained from the HSBC BACS guide - https://www.hsbcnet.com/-/media/hsbcnet/client-transition/xmlv3-uk-bacs-credit-guide.pdf
        r(pmtTpInf, 'SvcLvl', 'Cd', 'NURG');
      } else {
      ...
      if (this.method === PaymentInfoTypes.DirectDebit) {
        ...
        r(pmtInf, 'ReqdExctnDt', painVersion === 9 ? 'Dt' : null, this.requestedExecutionDate.toISOString().substr(0, 10));
      }
      ...
      if (this._painFormat === 'pain.001.001.09') {
        if (this[pullFrom + 'StreetName']) r(pstl, 'StrtNm', this[pullFrom + 'StreetName']);
        if (this[pullFrom + 'BuildingNumber']) r(pstl, 'BldgNb', this[pullFrom + 'BuildingNumber']);
        if (this[pullFrom + 'BuildingName']) r(pstl, 'BldgNm', this[pullFrom + 'BuildingName']);
        if (this[pullFrom + 'PostCode']) r(pstl, 'PstCd', this[pullFrom + 'PostCode']);
        if (this[pullFrom + 'TownName']) r(pstl, 'TwnNm', this[pullFrom + 'TownName']);
        if (this[pullFrom + 'CountrySubDivision']) r(pstl, 'CtrySubDvsn', this[pullFrom + 'CountrySubDivision']);
        if (this[pullFrom + 'Country']) r(pstl, 'Ctry', this[pullFrom + 'Country']);
      } else if (this[pullFrom + 'Street'] && this[pullFrom + 'City'] && this[pullFrom + 'Country']) {
        if (this[pullFrom + 'Country']) r(pstl, 'Ctry', this[pullFrom + 'Country']);
        if (this[pullFrom + 'Street']) r(pstl, 'AdrLine', this[pullFrom + 'Street']);
        if (this[pullFrom + 'City']) r(pstl, 'AdrLine', this[pullFrom + 'City']);
      }

      if (this[pullFrom + 'IBAN']) {
        r(pmtInf, emitterNodeName + 'Acct', 'Id', 'IBAN', this[pullFrom + 'IBAN']);
      } else {
        const accountValue = pullFrom === 'debtor' ? `${this[pullFrom + 'SORTCODE']}-${this[pullFrom + 'ACCOUNT']}`: this[pullFrom + 'ACCOUNT'];
        r(pmtInf, emitterNodeName + 'Acct', 'Id', 'Othr', accountValue);
      }

      if (this[pullFrom + 'BIC']) {
        r(pmtInf, emitterNodeName + 'Agt', 'FinInstnId', painVersion === 9 ? 'BICFI' : 'BIC', this[pullFrom + 'BIC']);
      } else {
  ...
  function SepaTransaction(painFormat,options) {
    this._painFormat = painFormat;
  ...
  SepaTransaction.prototype = {
    ...
    validate: function() {
      ...
      if (this._painFormat === 'pain.001.001.09') {
        ...
        assert_length(this[pullFrom + 'CountrySubDivision'], null, 70, pullFrom + 'CountrySubDivision');
        // Country code must be exactly 2 uppercase letters
        assert(this[pullFrom + 'Country'] && /^[A-Z]{2}$/.test(this[pullFrom + 'Country']), pullFrom + 'Country must be exactly 2 uppercase letters (ISO country code)');
      } else {
        // Unstructured address checks for other formats
        assert_length(this[pullFrom + 'Street'], null, 70, pullFrom + 'Street');
        assert_length(this[pullFrom + 'City'], null, 70, pullFrom + 'City');
    ...
    toXML: function(doc) {
      ...
      if (this._type === TransactionTypes.DirectDebit) {
        r(txInf, 'InstdAmt', this.amount.toFixed(2)).setAttribute('Ccy', this._currency);
        ...
        r(txInf, 'Amt', 'InstdAmt', this.amount.toFixed(2)).setAttribute('Ccy', this._currency);
      }

      if (this[pullFrom + 'BIC']) {
        r(txInf, recieverNodeName + 'Agt', 'FinInstnId', painVersion === 9 ? 'BICFI' : 'BIC', this[pullFrom + 'BIC']);
      } else if (this[pullFrom + 'SORTCODE']) {
        r(txInf, recieverNodeName + 'Agt', 'FinInstnId', 'ClrSysMmbId', 'MmbId', this[pullFrom + 'SORTCODE']);
      //   <FinInstnId>
      //     <ClrSysMmbId>
      //     <MmbId>123456</MmbId>
      //     </ClrSysMmbId>
      ...
      if (this[pullFrom + 'BIC']) {
        ...
        // This is different according to Loyds Bank depending on whether it is a debtor or creditor
        const accountValue = pullFrom === 'debtor' ? `${this[pullFrom + 'SORTCODE']}-${this[pullFrom + 'ACCOUNT']}`: this[pullFrom + 'ACCOUNT'];
        r(txInf, recieverNodeName + 'Acct', 'Id', 'Othr', 'Id', accountValue);
      }

      // Some banks require the Purp to be before the RmtInf
   ...
   *
   * @param cid        The Creditor ID to check.
   ...
   *
   * @param cid        The IBAN to calculate the checksum for.
   * @return            The corrected IBAN.
   */
   ...
   * @param sortCode             The Sort Code to check.
   * @return            True, if the Account Number and Sort Code Checksum Validates
   */
  function validateBACS(accountNumber,sortCode) {
    return new UkModulusChecking({ accountNumber, sortCode}).isValid();
  }

  /** assert that |str| is an IBAN */
  function assert_bacs(accountNumber,  sortCode, member) {
    if (!validateBACS(accountNumber,sortCode)) {
      throw new Error(member + ' has invalid Sort "' + sortCode + ' and Account "' + accountNumber + ' combination"');
    }
  }
  ...
  exports.validateBACS           = validateBACS;
